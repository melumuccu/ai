...
`install-to-cursor.sh` は `ai-ja-readability.env.example` を `~/.cursor/hooks/ai-ja-readability/ai-ja-readability.env` にコピーします。コピー先が既にあるときは上書きしません。

## 環境変数

値は `~/.cursor/hooks/ai-ja-readability/ai-ja-readability.env` に書きます。
