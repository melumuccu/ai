...
このセクションは、`melumuccu/ai` で skill を追加、改名、整理するときの前提です。
...
- このリポジトリ（`melumuccu/ai`）で skill を命名するときは、既存の記述どおり `kf-` の prefix を必ず付ける
- `kf-` の直後には必ず大カテゴリを置く
- 既存の外部 skill 名にはこの規約を後から強制しない
...
- `melumuccu/ai` では `kf-` の直後に、その他のリポジトリでは先頭に、大カテゴリを必ず置く
