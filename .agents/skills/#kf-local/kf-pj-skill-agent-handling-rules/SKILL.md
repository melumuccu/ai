...
skill は `kf-global` / `kf-global-commands` / `external` / `kf-local` の4バケットに分類する。

## 参照ファイル

- [references/overview.md](references/overview.md): 着手時: 基本原則・適用範囲・完了判定を確認するとき
- [references/repository-layout.md](references/repository-layout.md): 着手時: インストール前提・4バケット・必須 meta を確認するとき
...
1. [references/repository-layout.md](references/repository-layout.md) を読み、対象 skill のバケットと必須 meta を確認する
