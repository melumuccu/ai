...
- ユーザーが対応すべきタスクがある場合だけ、`## ユーザー対応タスク` を置く。タスクは実行順のチェックボックスで記載し、空セクションやダミーの項目は置かない
- PR description の順序は、最小サマリ、条件付きの `## ユーザー対応タスク`、`## レビュー用資料`、任意の `Closes #<issue-number>` とする。ユーザー対応タスクは HTML 資料と close 対象 issue より前に置く
