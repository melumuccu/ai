...
**撮影ツール：** スクリーンショットは **Playwright CLI** でヘッドレスブラウザを起動して取得する。
cursor-ide-browser MCP は挙動が不安定なため、原則として使わない。

**開発環境の UI：** dev mode 起動時に Astro の dev toolbar など、開発環境特有のコンポーネントがスクショに映り込むことは許容する。

**viewport：** 比較用途では `fullPage` ではなく固定 viewport で取得する。
before / after は同一サイズに揃える。
