...
description: Build a build-free, single-file interactive HTML document with text-selection comments, left-side term annotations for specialized vocabulary, SVG connectors, and localStorage persistence for user comments only (term annotations are static HTML, not persisted), PR walkthroughs, business flows, and non-coding docs. Use this whenever the user wants a one-page HTML explanation, interactive documentation, annotated guide, or shareable static page—even if they do not say "single-file HTML". Does not provide backend sync, authentication, or Worker deployment.
---
...
| [references/content-patterns.md](references/content-patterns.md)                             | PR 説明・業務フロー・非コーディング向けの構成、CDN 選定、視覚構造・可読性（§ 論理セクション分離・図表化・専門用語・左注釈・情報エンコード・アクセシビリティ） |
...
| 専門用語       | 初出を `mark[data-term-id]` でマークし、左 `[data-annotation-panel]` に著者定義を置く（詳細は [content-patterns.md](references/content-patterns.md) § 専門用語・左注釈） |
...
1. 用語定義を HTML 内静的データ（`#term-annotations` JSON 等）に埋め込み、左 `[data-annotation-panel]` に解説カードを配置する（localStorage は使わない）
1. 左注釈の SVG コネクタが mark 左→card 右を指すことを確認する
...
- [ ] 左 `[data-annotation-panel]` に用語解説カードがある（専門用語を使う文書の場合）
- [ ] 左注釈コネクタは mark 左→card 右を指す
- [ ] 用語データは HTML 内静的（`#term-annotations` 等）。localStorage しない
- [ ] 用語マークに badge またはテキストラベル等、色以外の識別手段がある
