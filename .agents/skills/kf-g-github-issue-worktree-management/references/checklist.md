...
- 計画を基に着手した場合、description を workflow に応じて更新したか。
  - HTML 配布あり（`kf-g-html-document-universal-single-file` で R2 アップロード済み）: 最小サマリと `## プランニング用資料` 配下の **`[v{N}](https://ai-html.hacksaw.work/<object-key>)`** のみを記載したか。計画全文・詳細は HTML に置き、description に複製していないか。既存 description は参考にし、最小サマリ + プランニング用資料形式へ統合・上書きしたか。
