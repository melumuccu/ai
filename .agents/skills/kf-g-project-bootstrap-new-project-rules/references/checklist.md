...
- self-hosted runner CI を **適用** とした場合、`.github/workflows/*.yml` の self-hosted job が `runs-on: [self-hosted, <repo-slug>]` になっているか。
- self-hosted runner CI を **適用** とした場合、host 側 runner に同名 label があるか（Settings → Actions → Runners）。
- self-hosted runner CI を **適用** とした場合、`kf-g-github-actions-self-hosted-ci-cache` の bootstrap step があるか。
