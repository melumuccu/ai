...
- `cqi` を使うなら、同じコンポーネント内に対応するコンテナ定義と `@container` 条件が存在することを確認する。
...
- 画面全幅を狙う場合は、固定 `inline-size` より `min-inline-size` や Intrinsic なレイアウトを先に検討する。
...
- 既存コードや AI 出力に問題がある場合は、単に書き換えるだけでなく、何が危ないのかも短く説明する。
