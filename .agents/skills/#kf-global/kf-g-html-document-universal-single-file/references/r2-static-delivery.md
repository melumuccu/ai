...
- 当該 HTML の object-key、put コマンド、公開 URL 確認は issue / PR description と本ファイルのワークフローで扱う
...
依頼の主題がインフラ操作である場合、HTML 本文（`[data-content-root]`）に **手動インフラ構築手順** を含める。**今回の構築内容（ユーザー申告を含む）** と **再現用クリック手順** を区別する。クリック履歴が取れない項目は捏造せず、一般的な再構築手順として記述する。当該 HTML の公開手順は [CLI 配布 runbook](#cli-配布-runbook本-repository) と [GitHub description へのリンク反映](#github-description-へのリンク反映) で扱う。
...
依頼主題がインフラ操作のときだけ、[content-patterns.md](content-patterns.md) のパターン D に従い、上記 runbook の要点（全体図、**手動インフラ構築手順**、**目視確認手順**、対象システムの CLI コマンド、失敗時、セキュリティ、操作ステータス）を `[data-content-root]` に記載する。当該 HTML の put 結果と確認済み公開 URL は issue / PR description の所定見出し（`## プランニング用資料` / `## レビュー用資料`）へ `[v{N}](https://ai-html.hacksaw.work/<object-key>)` として置く。本文の見出しと節は依頼主題の構成だけにする。
